...
# root_path = config.py.DATA_ROOT_PATH + "site_data/" + data_name
...
def gen_cleaned_data(input_file, cleaned_implicit_data_path, cleaned_explicit_data_path):
    ...
    with open(input_file, 'r') as file:
        ...
        for line in csv_reader:
            ...
            raw_item_id = line[3]
            if event == "view":
                csv_writer_implicit.writerow([raw_uid, raw_item_id, timestamp, 1])
            elif event == 'addtocart':
                # csv_writer_implicit.writerow([raw_uid, raw_item_id, timestamp, 1])
                # if event == 'addtocart':
                csv_writer_explicit.writerow([raw_uid, raw_item_id, timestamp, 2])
            elif event == 'transaction':
                csv_writer_explicit.writerow([raw_uid, raw_item_id, timestamp, 3])
            else:
                csv_writer_explicit.writerow([raw_uid, raw_item_id, timestamp, -1])
...
def main():
    # data_path = config.py.DATA_ROOT_PATH + "site_data/" + data_name + "/raw_data/events_purchase.csv"
    data_path = root_path + 'raw_data/new_events.csv'

    cleaned_implicit_data_path = root_path + '_implicit.filter.clean.txt'
    cleaned_explicit_data_path = root_path + '_explicit.filter.clean.txt'
    ...
    # data_preparation.combine_implicit_with_explicit_behaviors(cleaned_implicit_data_path, cleaned_explicit_data_path,
    #                                                 user_index_dict, item_index_dict, combined_data)
    # logging.info('--> Done, combine_implicit_with_explicit_behaviors')

    # div train test data without implicit in train
    data_preparation.div_train_test_data_without_implicit_in_train(combined_data, output_dir)
    logging.info('--> Done, div_train_test_data_without_implicit_in_train')

    # # div train test data with implicit in train
    # data_preparation.div_train_test_data_with_implicit_in_train(combined_data, output_dir)
    # logging.info('--> Done, div_train_test_data_with_implicit_in_train')

    subprocess.call(['bash', 'bin/split.sh', output_dir])
