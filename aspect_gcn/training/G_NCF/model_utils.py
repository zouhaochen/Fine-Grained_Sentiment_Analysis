...
from GCN.training.G_NCF import model_graph
...
class Utils:
    ...
    def load_data(self):
        ...
        interact_mat = data_utils.load_interact_matrix_NeuMF(self.root_path + "ITE/ratings_train.txt",
                                                             num_user,
                                                             num_item,
                                                             self.hyper_params["use_implicit"])
        test_data = data_utils.load_test_data(self.root_path + "ITE/ratings_train.txt")
        negative_data = data_utils.load_negative_data(self.root_path + "ITE/negative_test.txt")

        int_interact_mat = data_utils.load_int_interact_matrix(
            self.root_path + "ITE/ratings_train.txt", num_user, num_item)
        ...
        user_adj, user_deg = data_utils.construct_adj(row=user_adj.row, col=user_adj.col,
                                                      rate=user_adj.data, num=num_user,
        ...
        item_adj, item_deg = data_utils.construct_adj(row=item_adj.row, col=item_adj.col,
                                                      rate=item_adj.data, num=num_item,
                                                      max_deg=self.hyper_params["max_deg"])
    ...
    def train(self, model, data):
        ...
        with tf.Session(config=session_conf) as sess:
            ...
            for e in range(1, num_epoch + 1):
                ...
                r_test_loss = 0.0
                num_batch = 0
                train_path = self.root_path+'ITE/ratings_train.txt'
