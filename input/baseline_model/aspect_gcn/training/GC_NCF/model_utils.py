...
import sys
# sys.path.append('Colab_GITE/GCN')
from GCN.config import config
from GCN.data_preparation import data_utils
from GCN.training.GC_NCF import model_graph
...
class Utils:
    def __init__(self, hyper_params,
        ...
        # self.root_path + 'eval_results/{}/{}/{}_{}_{}_{}'
        # model_name/epoch/{num_factor}_{batch_size}_{lr}_{num_neg}

        self.model_path = ""
        # self.root_path + "models/{}/{}/{}_{}_{}_{}"
        # model_name/epoch/{num_factor}_{batch_size}_{lr}_{num_neg}
    ...
    def load_data(self):
        ...
        interact_mat = data_utils.load_interact_matrix_NeuMF(self.root_path + "without_implicit_in_train/_train.rating",
                                                             num_user,
                                                             num_item,
                                                             self.hyper_params["use_implicit"])
        # training_dict = data_utils.load_train_data(self.train_path + "scene_1/_explicit.train.rating")
        test_data = data_utils.load_test_data(self.root_path + "without_implicit_in_train/_test.rating")
        negative_data = data_utils.load_negative_data(self.root_path + "without_implicit_in_train/_test.negative")

        int_interact_mat, ui_adj, iu_adj = data_utils.load_int_interact_matrix_and_cross_adj_matrix(
            self.root_path + self.hyper_params["data_scene"] + "_train.rating", num_user, num_item,
            max_deg=self.hyper_params["max_deg"])
        user_adj = int_interact_mat.dot(int_interact_mat.transpose()).tocoo()
        user_adj, user_deg = data_utils.construct_adj(row=user_adj.row, col=user_adj.col, rate=user_adj.data,
                                                      num=num_user, max_deg=self.hyper_params["max_deg"])

        item_adj = int_interact_mat.transpose().dot(int_interact_mat).tocoo()
        item_adj, item_deg = data_utils.construct_adj(row=item_adj.row, col=item_adj.col, rate=item_adj.data,
                                                      num=num_item, max_deg=self.hyper_params["max_deg"])
    ...
    def set_hyper_params(self, hyper_params, save_eval_result, save_model):
        ...
        self.eval_result_path = self.root_path + "eval_results/{}/{}/{}_{}_{}_{}".format(hyper_params["model_name"],
        ...
        self.model_path = self.root_path + "models/{}/{}/{}_{}_{}_{}".format(hyper_params["model_name"],
                                                                             hyper_params["num_epoch"],
                                                                             hyper_params["num_factor"],
                                                                             hyper_params["batch_size"],
                                                                             hyper_params["lr"],
    ...
    def build_graph(self):
        ...
        """
        return model_graph.GC_NCF.create_model(self.hyper_params)
    ...
    def train(self, model, data):
        ...
        use_implicit = self.hyper_params["use_implicit"]
        ...
        with tf.Session(config=session_conf) as sess:
            ...
            for e in range(1, num_epoch + 1):
                ...
                r_test_loss = 0.0
                num_batch = 0
                partitioned_train_path = self.root_path + "without_implicit_in_train/partitioned_train_data/"
                ...
                for partition_name in sorted(os.listdir(partitioned_train_path)):
                    ...
                    # -----------------------> get train instances <-------------------------------
                    user_indices, item_indices, labels = data_utils.get_train_instances_partition_NeuMF(
                        partitioned_path, interact_mat, num_neg, num_item, use_implicit)
