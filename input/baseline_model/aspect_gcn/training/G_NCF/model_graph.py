...
from GCN.training.G_NCF import aggregators
...
class NeuMF:
    ...
    def create_model(hyper_params):
        ...
        with tf.device("/gpu:0"):
            ...
            embedding_weight = NeuMF.get_embedding_weight(user_adj_mat, item_adj_mat)
            ...
            # ------------------------------- Aggregate ----------------------------------
            gmf_user_agg = aggregators.GCNAggregator_1layer(input_dim=num_user, output_dim=num_factor_gmf,
                                                            name="gmf_weight_user")
            gmf_user_vec = gmf_user_agg(samples=user_samples)

            gmf_item_agg = aggregators.GCNAggregator_1layer(input_dim=num_item, output_dim=num_factor_gmf,
                                                            name="gmf_weight_item")
            gmf_item_vec = gmf_item_agg(samples=item_samples)

            mlp_user_agg = aggregators.GCNAggregator_1layer(input_dim=num_user, output_dim=num_factor_mlp,
                                                            name="mlp_weight_user")
            mlp_user_vec = mlp_user_agg(samples=user_samples)

            mlp_item_agg = aggregators.GCNAggregator_1layer(input_dim=num_item, output_dim=num_factor_mlp,
                                                            name="mlp_weight_item")
